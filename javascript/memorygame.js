...
const startGame = () => {
  ...
  state.loop = setInterval(() => {
    state.totalTime++;
    selectors.moves.innerText = `${state.totalFlipps} moves`;
...
const flipCard = (card) => {
  ...
  if (state.flippedCards === 2) {
    ...
    if (
      flippedCards[0].querySelector(".card-back").innerText ===
      flippedCards[1].querySelector(".card-back").innerText
    ) {
...
<span class="win-text">
       You won!<br/>
       with <span class="highlight">${state.totalFlipps}</span>
       moves<br/>
